...
public class SolutionPrinter {
        ...
        for (VehicleRoute route : list) {
            ...
            for (TourActivity act : route.getActivities()) {
                ...
                out.format(leftAlgin, routeNu, getVehicleString(route), act.getName(), jobId, Math.round(act.getArrTime()),
                    Math.round(act.getEndTime()), Math.round(costs), getGroupId(act));
                prevAct = act;
            }
    ...
    private static int getGroupId(TourActivity act) {
        int groupId = -1;
        if(act instanceof TourActivity.JobActivity){
            Job job = ((TourActivity.JobActivity) act).getJob();
            if(job instanceof Shipment){
                groupId = ((Shipment) job).getTaipingGroup();
            }
        }
        return groupId;
    }
